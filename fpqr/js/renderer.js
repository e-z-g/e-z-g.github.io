...
function renderCanvas() {
    ...
    for (const [id, val] of Object.entries(updates)) {
        if (domCache[id] !== val) { safeSetText(id, val); domCache[id] = val; }
    }
    ...
    const renderM = (canId, matrix) => {
        ...
        if (can.width !== cSz) can.width = can.height = cSz;
        ...
        const isModuleObscured = (r, c) => {
            ...
            const clX = Math.max(mLeft, Math.min(lC, mRight)), clY = Math.max(mTop, Math.min(lC, mBottom));
            return ((lC-clX)**2 + (lC-clY)**2) < (tSz/2)**2;
        };
        ...
        const shouldRenderData = (r, c) => {
            if (!isDarkData(r, c)) return false;
            if (isModuleObscured(r, c) && omit && !heatmap) return false;
            return true;
        };
        ...
        if (colorStyle === 'image') {
            let activeImage = colorMapImage, isDynamic = false;
        ...
        if (densityMapStyle === 'image') {
            let activeImage = densityMapImage, isDynamic = false;
        ...
        for (let r = 0; r < gSz; r++) {
            for (let c = 0; c < gSz; c++) {
                ...
                if (shape === 'solid') {
                    ...
                    const ov = 0.6;
                    const cR = cx+cell+(ri?ov:0), cB = cy+cell+(b?ov:0), cL = cx-(l?ov:0), cT = cy-(t?ov:0);
                    ...
                    ctx.moveTo(cx+rad,cy); ctx.lineTo(cx+cell,cy); ctx.lineTo(cx+cell,cy+cell-rad);
                    ctx.quadraticCurveTo(cx+cell,cy+cell,cx+cell-rad,cy+cell);
                    ctx.lineTo(cx,cy+cell); ctx.lineTo(cx,cy+rad);
                    ctx.quadraticCurveTo(cx,cy,cx+rad,cy); ctx.fill();
                } else if (shape === 'diamond') {
                    const scale = Math.max(0.1, dB/100);
                    const dR = (cell/2)*scale, cxc = cx+cell/2, cyc = cy+cell/2;
        ...
        if (logoShape !== 'none') {
            if (!heatmap) {
                ...
                ctx.fillStyle = 'rgba(255,255,255,0.1)'; ctx.strokeStyle = 'rgba(255,255,255,0.5)';
                ctx.lineWidth = 4; ctx.setLineDash([10, 10]);
                if (logoShape === 'circle') {
                    ctx.beginPath(); ctx.arc(lC, lC, tSz/2, 0, Math.PI*2); ctx.fill(); ctx.stroke();
                } else {
                    ctx.fillRect(pXy, pXy, tSz, tSz); ctx.strokeRect(pXy, pXy, tSz, tSz);
                }
                ctx.setLineDash([]);
            }
        }
    ...
    if (mini) {
        const mSz = 1024;
        if (mini.width !== mSz) mini.width = mini.height = mSz;
